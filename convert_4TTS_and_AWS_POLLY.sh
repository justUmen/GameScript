				...
				while read LINE; do
					...
					if [[ "$COMMENT" == "" ]]; then
						...
						if [[ "$QUIZ" == "" ]]; then
							...
							#~ if [[ "$QUESTION" == "" ]]; then
								TTS_LINE=`echo $LINE | sed 's#$(pwd)#/home/example/House#' | sed 's#\${learn}..\${reset}# DOT DOT #' | sed 's#$HOME#/home/example#g' | sed 's#mkdir#M.K.D.I.R.#g' | sed "s#'M.K.D.I.R.'#M.K.D.I.R.#" | sed 's#rmdir#R.M.D.I.R.#g' | sed "s#'/'# SLASH #g" | sed "s#SLASH \.#SLASH.#g"| sed "s#SLASH ,#SLASH,#g" | sed 's#/# SLASH #g' | sed 's#\${learn}##g' | sed 's#\${voc}##g' | sed 's#\${reset}##g' | sed 's#\${codeFile}##g' | sed 's#\${codeError}##g' | sed 's#\${code}##g' | sed 's#^+##g'`
								echo " ===> $TTS_LINE"
